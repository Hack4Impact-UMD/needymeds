...
describe("GET /dsnt/price-by-ndc", () => {
  const host = "https://argusprod.dstsystems.com";
  afterEach(() => nock.cleanAll());

  it("200 happy path", async () => {
    nock(host).get(/PharmacyPricing/).reply(200, { foo: "bar" });
    const res = await request(app).get("/dsnt/price-by-ndc")
...
describe("DSNTService.priceByNDC", () => {
  ...
  it("fails after exhausting retries with 5xx", async () => {
    const scope = nock(host)
      .get("/pharmacy-drug-pricing/1.0/service/PharmacyPricing").times(3)
      .query({ quantity: "100", ndc: "59148000713", radius: "100", zipCode: "10003" })
      .reply(503, {});
...
describe("DSNTService.priceByNdcAndNpiListQuery", () => {
  const service = new DSNTService();
  const host = "https://argusprod.dstsystems.com";
  afterEach(() => nock.cleanAll());
  ...
  it("returns pricing for npilist single", async () => {
    ...
    const data = await service.priceByNdcAndNpiListQuery({
      ndc: "59148000713", quantity: 100, npilist: "1326064445",
    });
    expect(data).toEqual({ DrugPricing: [{ npi: "1326064445", ndc: "59148000713", price: "2087.39" }] });
  });
  ...
  it("returns pricing for multiple npis", async () => {
    ...
    const data = await service.priceByNdcAndNpiListQuery({
      ndc: "00591405289", quantity: 45, npilist: "1013431949,1326064445",
    });
    expect(data).toEqual({ DrugPricing: [] });
  });

  it("validates bad npi", async () => {
    await expect(service.priceByNdcAndNpiListQuery({ ndc: "00591405289", quantity: 45, npilist: "abc" }))
