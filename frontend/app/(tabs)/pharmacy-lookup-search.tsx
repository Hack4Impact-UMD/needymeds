...
const PharmacyLocatorScreen = () => {
  ...
  const isSaved = (pharmacy: Pharmacy) => {
    return savedPharmacies.some(
      (sp) => sp.pharmacy_name === pharmacy.pharmacyName && sp.street === pharmacy.pharmacyStreet1 && sp.city === pharmacy.pharmacyCity
    );
  };
          ...
          <ScrollView showsVerticalScrollIndicator={false} onTouchStart={Keyboard.dismiss}>
            {loading ? (
              ...
              filteredPharmacies.map((pharmacy) => {
                ...
                return (
                  <TouchableOpacity key={key} onPress={() => setSelectedPharmacy(pharmacy)} activeOpacity={0.7}>
                    <View style={styles.resultRow}>
                      <TouchableOpacity onPress={() => toggleStar(pharmacy)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                        <MaterialCommunityIcons
                      ...
                      <View style={styles.resultText}>
                        <Text style={styles.resultTitle} numberOfLines={1}>{pharmacy.pharmacyName}</Text>
                        <Text style={styles.resultSubtitle} numberOfLines={2}>{pharmacy.pharmacyStreet1}, {pharmacy.pharmacyCity}</Text>
                      </View>
                      <View style={styles.resultRight}>
                        <Text style={styles.resultMiles}>{Number(pharmacy.distance).toFixed(1)}mi</Text>
